...
config_stack=`jq -r .stack//empty ${computed_file_path}`
...
config_s3_packaged_filename=`jq -r .s3.packaged_filename//empty ${computed_file_path}`
...
[ -z $config_stack ] && die "Required config 'stack' Not Defined" || yell "app = ${config_stack}"
...
[ -z $config_s3_packaged_filename ] && die "Required config 's3.packaged_filename' Not Defined" || yell "s3.packaged_filename = ${config_s3_packaged_filename}"
...
case $1 in
    ...
    'package')
        ...
        spackage="${scmd} package --output-template-file ${config_s3_packaged_filename} --s3-bucket ${config_s3_bucket_name} --s3-prefix ${config_s3_bucket_prefix} --profile $4"
        try $spackage
        ;;
    'deploy')
