...
case $1 in
    'build')
        ...
        yell "SAM: Building Frontend application"
        suibuild="npm run build"
        try $suibuild
        ;;
    'package')
    ...
    'deploy')
        ...
        ;;
    'upload')
        yell "SAM: Upload UI application to S3"
        supload="aws s3 cp dist s3://${config_stack} --recursive --profile $4 --metadata test=testing"
        try $supload

        yell "SAM: Invalidate Cloudfront Distribution"
        # sinvalidate="aws cloudfront create-invalidation --distribution-id ${distribution_id} --paths "/*""
        # try $sinvalidate
        ;;
    *)
